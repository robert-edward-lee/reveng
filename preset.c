...
main(int argc, char *argv[]) {
	...
	model_t a = {PZERO, PZERO, P_BE, PZERO, PZERO, NULL};
	model_t b = {PZERO, PZERO, P_BE, PZERO, PZERO, NULL};
	...
	while(plen(a.spoly)) {
		...
		if(plen(b.spoly) && mcmp(&a, &b) >= 0) {
			fprintf(stderr, "%s: coding error.  Models "
				"\"%s\" and \"%s\" out of order.\n",
				myname, a.name, b.name);
			mfree(&b);
			mfree(&a);
			exit(EXIT_FAILURE);
		}
	...
	while(alias[0].name && alias[1].name) {
		if(strcmp(alias[0].name, alias[1].name) >= 0) {
			fprintf(stderr, "%s: coding error.  Aliases "
				"\"%s\" and \"%s\" out of order.\n",
				myname, alias[0].name, alias[1].name);
			exit(EXIT_FAILURE);
		}
		++alias;
	}
